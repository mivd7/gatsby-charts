    ...
    <Layout>
      ...
      >
        {countries.map(country => (
          <MenuItem
            id={country}
            value={edges.find(e => e.node.Country === country)}
          >
            {country}
          </MenuItem>
        ))}
      ...
      {madeSelection && (
        <>
          <h2>Now pick the stats you wish to show</h2>
          <List>
            {filteredHeaders.map(header => (
              <ListItem button>
                <ListItemIcon>
                  <Checkbox
                    onChange={e =>
                      setPickedProperties(arr => [...arr, e.target.value])
                    }
                    value={header}
                    inputProps={{
          ...
          <h2>
            Finally select what type chart you want to use to display these
            stats
          </h2>
          <Select
            value={chartType}
          ...
          >
            {chartTypes.map(type => (
              <MenuItem id={chartTypes.indexOf(type)} value={type}>
                {type}
              </MenuItem>
            ))}
          </Select>
      ...
      {isComplete && (
        <HighchartsReact
          ...
          options={customizedOptions(options)}
        />
      )}
